...
# ================================================================
# Phase 3: Assign 5-zone labels — FIXED (distance-only assignment)
# ================================================================
# KEY CHANGE: All 5 zones assigned by distance quantiles ONLY.
# NEPC_UP score is used for VALIDATION (confirming the gradient
# exists) but NOT for zone membership gating.
#
# Zone boundaries (all distance-based):
#   Interface:     dist == 0
#   Proximal NE:   0 < dist ≤ Q20 (closest 20% of non-interface)
#   Distal NE:     Q20 < dist ≤ Q45 (next 25%)
#   Epithelial:    Q45 < dist ≤ Q75 (middle 30%)
#   Necrotic Core: dist > Q75 (farthest 25%)
# ================================================================
cat("\n  Phase 3: Assigning 5-zone labels (distance-only)...\n")

# Recompute zone boundaries — wider bands, no score gating
...
for (cn in names(cond_dist_data)) {
  ...
  q20 <- quantile(d_ni, 0.20, na.rm = TRUE)
  q45 <- quantile(d_ni, 0.45, na.rm = TRUE)
  q75 <- quantile(d_ni, 0.75, na.rm = TRUE)

  # Ensure minimum separation between boundaries
  d_range <- max(d_ni, na.rm = TRUE) - min(d_ni, na.rm = TRUE)
  min_gap <- max(1.0, d_range * 0.05)  # at least 1 array unit or 5% of range

  if ((q45 - q20) < min_gap) {
    q20 <- quantile(d_ni, 0.15, na.rm = TRUE)
    q45 <- quantile(d_ni, 0.50, na.rm = TRUE)
  }
  if ((q75 - q45) < min_gap) {
    q75 <- quantile(d_ni, 0.80, na.rm = TRUE)
  }
  ...
  cat(paste0("    ", cn, ":\n",
             "      Interface       = dist == 0\n",
             "      Proximal NE     = 0 < d ≤ ", round(q20, 2), "\n",
             "      Distal NE       = ", round(q20, 2),
             " < d ≤ ", round(q45, 2), "\n",
             "      Epithelial      = ", round(q45, 2),
             " < d ≤ ", round(q75, 2), "\n",
             "      Necrotic Core   = d > ", round(q75, 2), "\n"))
}

rm(cond_dist_data); invisible(gc())

# Assign zones — pure distance, no score gating
for (sn in successfully_loaded) {
  ...
  if (!is.null(zb)) {
    ...
    if (length(ni) > 0) {
      ...
      # Zone 2: Proximal NE — closest non-interface spots
      zone5[ni[d_ni <= zb$proximal_max]] <- "Proximal_NE"

      # Zone 3: Distal NE — next distance band
      zone5[ni[d_ni > zb$proximal_max &
                 d_ni <= zb$distal_ne_max]] <- "Distal_NE"

      # Zone 4: Epithelial — middle distances
      zone5[ni[d_ni > zb$distal_ne_max &
                 d_ni <= zb$necrotic_min]] <- "Epithelial"

      # Zone 5: Necrotic Core — farthest
      zone5[ni[d_ni > zb$necrotic_min]] <- "Necrotic_Core"
    }
  } else {
  ...
  # Verify that NEPC_UP score decreases across zones
  # (this is now a TEST of the model, not a zone assignment criterion)
  ...
  if (!is.null(ne) && !all(is.na(ne))) {
    ...
    # Check the key prediction: Interface > Proximal > Distal
  ...
  # Backward-compatible interface_zone
  legacy_zone <- rep("Other", length(d))
